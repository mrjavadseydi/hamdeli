            ...
            <div class="card card-primary card-outline">
                ...
                <div class="table-responsive p-1 " style="overflow: hidden">
                    <table class="table table-striped" id="table-data2">
                        ...
                        <tbody>
                        @foreach($group as $l =>$val)
                            <tr>
                                <td>{{$l+1}}.</td>
                                <td>{{\App\Models\Plan::whereId($val->plan_id)->first()->title}}</td>
                                <td>
                                    {{\App\Models\NeederPlan::wherePlanId($val->plan_id)->count()}}
                                </td>
                                <td>
                                    {{\App\Models\DonatorPlan::wherePlanId($val->plan_id)->count()}}
                                </td>
                                <td>
                                    @if(\App\Models\Plan::whereId($val->plan_id)->first()->status==0)
                                    <span class="badge badge-info">
                                       در حال اماده سازی
                                    </span>
                                @elseif(\App\Models\Plan::whereId($val->plan_id)->first()->status==1)
                                <span class="badge badge-info">
                                    ارجاع به مجری
                                </span>
                                @elseif(\App\Models\Plan::whereId($val->plan_id)->first()->status==2)
